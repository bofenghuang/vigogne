...
def get_args():
    ...
    parser.add_argument("--num_preprocessing_workers", type=int, default=32, help="")
...
def main():
    ...
    if os.path.exists(args.output_file):
        ...
        dataset = dataset.filter(
            lambda x: x not in existing_values, input_columns=args.id_column_name, num_proc=args.num_preprocessing_workers
        )
    ...
    if args.prompt_file is not None:
        ...
        # dataset = dataset.map(
        #     lambda x: {"pre_query_str": prompt_template.format(text=x[args.instruct_column_name])},
        #     num_proc=args.num_preprocessing_workers,
        # )

        num_final_criteria = 3

        def process_function(instruction):
            # m = re.search(r"<methods>\n(.+)\n</methods>", prompt_template, flags=re.DOTALL).groups()[0]
            # criteria_list = m.split("\n")
            m = re.search(r"<methods>\n(.+)\n</methods>", prompt_template, flags=re.DOTALL)
            matched_str = m.group()
            criteria_list = m.groups()[0].split("\n")
            final_criteria = "\n".join(random.sample(criteria_list, num_final_criteria))
            # final_prompt_template = re.sub(m, final_criteria, prompt_template)
            final_prompt_template = re.sub(re.escape(matched_str), final_criteria, prompt_template)
            # return final_prompt_template.format(prompt=instruction)
            # return re.sub(r"{prompt}", instruction, final_prompt_template)
            return final_prompt_template + instruction

        dataset = dataset.map(
            lambda x: {"pre_query_str": process_function(x[args.instruct_column_name])},
            num_proc=args.num_preprocessing_workers,
        )
    ####################################################################################################
    ...
    #     data.append({"topic": topic_name, "pre_query_str": prompt})
